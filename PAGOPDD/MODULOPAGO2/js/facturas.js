...
`)
  ...
  estado.facturas = (data || []).map(f => ({
    ...f,
    descuento_cfdi: Number(f.descuento || 0),
    descuento: null
  }));
  pintarFacturas();
}
  ...
  tbody.innerHTML = estado.facturas.map(f => {
    ...
    return `
      <tr>
        ...
        <td><a class="link-detalle" href="detalle.html?uuid=${encodeURIComponent(f.uuid_cfdi || '')}" target="_blank">Ver detalle</a></td>
      </tr>`;
  }).join('');
  recalcularPago();
}
