...
function actualizarResumenSeleccion(subTotal=0,totalDescuentos=0){
  const count=document.querySelectorAll('#facturasBody input[type=checkbox]:checked').length;
  const totalFinal=Math.max(0,Number(subTotal||0)-Number(totalDescuentos||0));
  ['selCount1','selCount2'].forEach(id=>{const el=document.getElementById(id);if(el)el.textContent=String(count)});
  ['selAmount1','selAmount2'].forEach(id=>{const el=document.getElementById(id);if(el)el.textContent=money(subTotal)});
  const desc=document.getElementById('selDiscount2'); if(desc)desc.textContent=money(totalDescuentos);
  const pay=document.getElementById('selPayable2'); if(pay)pay.textContent=money(totalFinal);
}

export function actualizarResumenSeleccionBruto(){
  const checks=[...document.querySelectorAll('#facturasBody input[type=checkbox]:checked')];
  const total=checks.reduce((sum,cb)=>{
    const f=state.facturas.find(x=>String(x.id)===String(cb.dataset.id));
    return sum+Number(f?.total||0);
  },0);
  actualizarResumenSeleccion(total,0);
}
...
export async function recalcular(){
  ...
  if(!haySeleccion){setTotales(0,0);actualizarResumenSeleccion(0,0);return}
  if(haySeleccion&&!fechaLista){
    body.innerHTML='<tr><td colspan="11"><span class="pill pill-no">Selecciona la fecha de lista primero</span></td></tr>'; const bruto=construirArticulosSeleccionados(); setTotales(bruto,0); actualizarResumenSeleccion(bruto,0); return;
  }
  ...
  actualizarResumenSeleccion(subTotal,totalDescuentos);
}
...
function setTotales(subTotal,totalDescuentos){
  ...
  actualizarResumenBanco(totalFinal);
}

function actualizarResumenBanco(totalFinal){
  const box=document.getElementById('resumenBanco');
  if(!box)return;
  const seleccionadas=[...document.querySelectorAll('#facturasBody input[type=checkbox]:checked')]
    .map(cb=>state.facturas.find(f=>String(f.id)===String(cb.dataset.id)))
    .filter(Boolean);
  if(!seleccionadas.length){box.value='';return}
  const lineas=seleccionadas.map(f=>{
    const folio=[f.serie,f.folio].filter(Boolean).join(' ').trim() || String(f.uuid_cfdi||'').slice(0,8);
    return `${folio} - ${money(f.total)}`;
  });
  lineas.push(`TOTAL FACTURAS: ${money(seleccionadas.reduce((s,f)=>s+Number(f.total||0),0))}`);
  if(Number(totalFinal)!==seleccionadas.reduce((s,f)=>s+Number(f.total||0),0)){
    lineas.push(`DESCUENTOS: ${money(Math.max(0,seleccionadas.reduce((s,f)=>s+Number(f.total||0),0)-Number(totalFinal||0)))}`);
  }
  lineas.push(`TOTAL A PAGAR: ${money(totalFinal)}`);
  box.value=lineas.join('\n');
}
