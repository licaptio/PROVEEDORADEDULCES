...
async function cargarPago(id){
  ...
  $('facturasBody').innerHTML='<tr><td colspan="10" class="empty">Cargando reporte...</td></tr>';
  $('reporte').scrollIntoView({behavior:'smooth',block:'start'});
  const {data,error}=await sb.from('pagos_proveedor').select('*').eq('id',id).single();
  if(error){ $('facturasBody').innerHTML=`<tr><td colspan="10" class="empty">Error: ${esc(error.message)}</td></tr>`; return; }
  pagoActual=data;
...
function renderReport(p){
  ...
  if(!facturas.length){
    $('facturasBody').innerHTML='<tr><td colspan="10" class="empty">Este pago no contiene detalle de facturas.</td></tr>';
  }else{
