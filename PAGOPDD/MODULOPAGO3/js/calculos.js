...
export function recalcularPago(){
  ...
  const totalBancoPago = $('totalBancoPago');
  if(totalBancoPago) totalBancoPago.textContent = money(totalFinal);
...
function pintarResumenPagoFinal(){
  ...
  const nombreProveedorBanco = $('nombreProveedorBanco');
  if(nombreProveedorBanco) nombreProveedorBanco.textContent = estado.proveedorNombre || 'Proveedor no seleccionado';

  const rfcProveedorBanco = $('rfcProveedorBanco');
  if(rfcProveedorBanco) rfcProveedorBanco.textContent = estado.rfcProveedor ? `RFC: ${estado.rfcProveedor}` : '';
