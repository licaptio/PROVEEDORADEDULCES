...
function abrirModalCobro() {
            ...
            ${["Efectivo 1", "Efectivo 2", "Tarjeta", "Transferencia"].map((label, i) => `
              <label data-campo-pago="${i + 1}" style="display:none;font-size:15px;font-weight:800;">
                ...
                <div style="position:relative;margin-top:5px;">
                  <span style="position:absolute;left:12px;top:50%;transform:translateY(-50%);font-size:20px;font-weight:900;color:#555;pointer-events:none;">$</span>
                  <input id="pago${i + 1}" type="number" step="0.01" min="0" inputmode="decimal" lang="es-MX" placeholder="0.00" autocomplete="off"
                    style="width:100%;box-sizing:border-box;height:50px;font-size:22px;font-weight:800;text-align:center;padding-left:28px;border:2px solid #aaa;border-radius:10px;">
                </div>
              </label>
            `).join("")}
          </div>
  ...
  function seleccionarTipoPago(tipo) {
    ...
    if (tipo === "EFECTIVO") {
      ...
      modal.querySelector('[data-etiqueta-pago="1"]').textContent = "Efectivo recibido (MXN)";
      setTimeout(() => {
        entradasPago[0]?.focus();
        entradasPago[0]?.select();
      }, 30);
  ...
  entradasPago.forEach(input => {
    input?.addEventListener("input", recalcularCambio);
    input?.addEventListener("keydown", (event) => {
      ...
      // Enter desde el campo de pago equivale a confirmar el cobro.
  ...
  // Flujo rápido de caja: al abrir COBRO queda Efectivo seleccionado
  // y el cursor listo para capturar el importe recibido.
