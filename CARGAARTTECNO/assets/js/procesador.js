...
const CENTINELA_REF = ["almacenes", "abarrotespdd", "configuracion", "catalogo_version"];
...
async function avisarCambioCatalogo(totalCambios) {
  if (!Number.isFinite(totalCambios) || totalCambios <= 0) {
    return { actualizado: false, motivo: "Sin cambios reales" };
  }

  const ref = doc(db, ...CENTINELA_REF);

  await setDoc(ref, {
    version: increment(1),
    actualizadoEn: serverTimestamp(),
    origen: "CARGAARTTECNO",
    cambiosUltimoProceso: totalCambios
  }, { merge: true });
...
async function procesarProductos(texto) {
  ...
  for (const nuevo of map.values()) {
    ...
    // AHORRO FIREBASE:
    // Si el producto realmente no cambió, no hacemos una escritura sólo para
    // marcarlo como "verificado". La comparación ya ocurrió en memoria.
    // Esto evita miles de escrituras innecesarias y también evita despertar
    // a las apps receptoras por productos sin cambios reales.
    if (!tieneCambios && !fueReactivado) {
      if (ultimaVerificacion === hoy) omitidosYaVerificadosHoy++;
      continue;
    }
...
async function procesar(handles) {
  ...
  // Sólo las escrituras reales de catálogo deben avisar a las apps.
  // Las verificaciones sin cambios ya no generan escritura en Firestore.
  const cambiosReales = totalActualizados;

  // El centinela se toca UNA SOLA VEZ por proceso, aunque cambien cientos de productos.
  // Si no hubo cambios reales, no hay escritura adicional ni se despiertan los teléfonos.
  const centinela = await avisarCambioCatalogo(cambiosReales);
  ...
  return {
    ...
    resumen: {
      ...
      cambiosReales,
      centinelaActualizado: centinela.actualizado,
      archivosReset: true
    },
    detalle: [
      {
      ...
      {
        etapa: "Aviso automático",
        estado: "OK",
        detalle: centinela.actualizado
          ? `Centinela incrementado una sola vez por ${cambiosReales} cambio(s) real(es)`
          : "Sin cambios reales: no se modificó el centinela"
      },
      {
        etapa: "Reset",
        estado: "OK",
