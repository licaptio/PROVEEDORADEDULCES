...
document.body.addEventListener("click", async e => {
  ...
  if (e.target.id === "chkTodasObservadas") {
    document.querySelectorAll(".chkFacturaObservada").forEach(chk => {
      chk.checked = e.target.checked;
    });
    actualizarContadorSeleccionadas();
  }

  if (e.target.classList.contains("chkFacturaObservada")) {
    actualizarContadorSeleccionadas();
  }
...
async function accionDesmarcar() {
  ...
  const checksSeleccionados = Array.from(
    document.querySelectorAll(".chkFacturaObservada:checked")
  );

  if (checksSeleccionados.length > 0) {
    const total = checksSeleccionados.length;

    if (!confirm(`¿Desmarcar / confirmar ${total} factura(s) seleccionada(s)?`)) {
      return;
    }

    mostrarCargaModal(`Confirmando ${total} factura(s)...`);
    ...
    for (const chk of checksSeleccionados) {
      ...
      const factura = observadas.find(f => f.uuid_cfdi === uuid);
      ...
      const { error } = await desmarcarFactura(factura, notaFinal, usuario);

      if (error) {
        console.error("Error al confirmar factura", uuid, error);
        errores++;
        continue;
      }

      await insertarHistorial(factura, "DESMARCAR", notaFinal, usuario);
      correctas++;
    }

    await refrescarObservadas();

    if (facturaActual?.uuid_cfdi) {
      const { data } = await buscarFacturaPorUUID(facturaActual.uuid_cfdi);
      facturaActual = data;
      if (facturaActual) {
        const hist = await cargarHistorial(facturaActual.uuid_cfdi);
        historialActual = hist.data || [];
        vistas.editor.innerHTML = renderEditor(facturaActual, historialActual);
      }
    }
    ...
    if (errores > 0) {
      mostrarToast(`${correctas} confirmadas. ${errores} con error.`);
    } else {
      mostrarToast(`${correctas} factura(s) desmarcada(s) / confirmada(s).`);
    }
  ...
  if (!facturaActual) {
    mostrarToast("Abre una factura o selecciona varias de la lista.");
    return;
  }
...
function actualizarContadorSeleccionadas() {
  const total = document.querySelectorAll(".chkFacturaObservada:checked").length;
  const etiqueta = document.getElementById("seleccionadasObservadas");

  if (etiqueta) {
    etiqueta.textContent = total
      ? `${total} seleccionada(s)`
