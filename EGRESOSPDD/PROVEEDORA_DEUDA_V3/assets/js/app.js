...
function renderDebtView(){
  ...
  const desktopRows = state.shown.map(row => {
    ...
    return `<tr class="${overdue}" data-uuid="${esc(row.uuid_cfdi)}">
      ...
      <td><a class="uuid-link" href="../detalle.html?uuid=${encodeURIComponent(row.uuid_cfdi)}" target="_blank" title="${esc(row.uuid_cfdi)}">${esc(row.uuid_cfdi || "—")}</a></td>
  ...
  const mobileCards = state.shown.map(row => {
    ...
    return `<article class="invoice-card ${overdue}" data-uuid="${esc(row.uuid_cfdi)}">
      ...
      <div class="invoice-grid">
        ...
        <div class="invoice-field"><small>UUID</small><a href="../detalle.html?uuid=${encodeURIComponent(row.uuid_cfdi)}" target="_blank">${esc(row.uuid_cfdi || "—")}</a></div>
      </div>
...
document.addEventListener("visibilitychange", () => { if(document.visibilityState === "visible") loadDebt({quiet:true}); });
window.addEventListener("beforeunload", () => { if(state.realtime) db.removeChannel(state.realtime); clearInterval(state.fallback); });
